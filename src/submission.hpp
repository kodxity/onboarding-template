...
inline void copy_boundaries(const Grid& old_grid, Grid& new_grid) {
    ...
    for (std::size_t i = 1; i + 1 < rows; ++i) {
        ...
        new_data[i * stride + cols - 1] = old_data[i * stride + cols - 1];
    }
}
...
#pragma omp parallel for schedule(static)
    for (std::size_t i = 1; i < rows - 1; i++) {
        ...
        for (; j < 4 && j < cols - 1; ++j) {
            res[j] =
                0.5 * curr_row[j] + 0.125 * (prev_row[j] + next_row[j] +
                                             curr_row[j - 1] + curr_row[j + 1]);
        }

        for (; j + 4 <= cols - 1; j += 4) {
            const __m256d center = _mm256_load_pd(curr_row + j);
            const __m256d vertical = _mm256_add_pd(
                _mm256_load_pd(prev_row + j), _mm256_load_pd(next_row + j));
            const __m256d horizontal =
                _mm256_add_pd(_mm256_loadu_pd(curr_row + j - 1),
                              _mm256_loadu_pd(curr_row + j + 1));
            const __m256d result =
                _mm256_fmadd_pd(_mm256_add_pd(vertical, horizontal), eighth,
